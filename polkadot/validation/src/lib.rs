...
use abc_primitives::{Hash, Block, BlockId, BlockNumber, Header, SessionKey, AuraId};
	...
	C::Api: ParachainHost<Block> + BlockBuilderApi<Block>,
{
	fn propose_with(&mut self, candidates: Vec<AttestedCandidate>) -> Result<Block, Error> {
		...
		inherent_data.put_data(abc_runtime::PARACHAIN_INHERENT_IDENTIFIER, &candidates).map_err(Error::InherentError)?;
